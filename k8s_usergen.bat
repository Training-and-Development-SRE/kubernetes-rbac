...
openssl x509 -req -in %_OUTPUTDIR%\%_USERNAME%.csr -CA %_CA_LOCATION%\.minikube\ca.crt -CAkey %_CA_LOCATION%\.minikube\ca.key -CAcreateserial -out %_OUTPUTDIR%\%_USERNAME%.crt -days 500
timeout /t 1

kubectl config --kubeconfig=%_OUTPUTDIR%\config set-cluster minikube --certificate-authority=%_CA_LOCATION%\.minikube\ca.crt --server=%_SERVERURL% --embed-certs=true
